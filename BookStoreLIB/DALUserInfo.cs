...
namespace BookStoreLIB
{
    public class DALUserInfo
    {
        ...
        private static string ResolveConn()
        {
            ...
            // This allows us to use the remote DB if configured
            ...
            // Fallback: use environment variables (for remote Agile server)
        ...
        public int LogIn(string userName, string password)
        {
            ...
            using (var cmd = new SqlCommand("SELECT UserID FROM UserData WHERE UserName=@UserName AND Password=@Password", conn))
            {
        ...
        // -------------------- GET MANAGER AND TYPE --------------------
        public (bool IsManager, string Type) GetManagerAndType(int userId)
        {
            using (var conn = new SqlConnection(ResolveConn()))
            using (var cmd = new SqlCommand("SELECT CAST(Manager AS bit), [Type] FROM UserData WHERE UserID=@id", conn))
            {
        ...
        // -------------------- REGISTER USER (MERGED FROM YOUR BRANCH) --------------------
        public bool RegisterUser(string fullName, string username, string password)
        {
            using (var conn = new SqlConnection(ResolveConn()))
            {
                ...
                using (var checkCmd = new SqlCommand("SELECT COUNT(*) FROM UserData WHERE UserName = @UserName", conn))
                {
